    ...
    return (
      <section className="py-16 border-b border-border">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-sm text-muted-foreground">Não foi possível carregar os mais pedidos agora. Tente novamente em instantes.</p>
        </div>
      </section>
    );
  }

  if (!carregando && hamburgueres.length === 0) return null;
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {carregando
            ? Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="rounded-2xl bg-secondary border border-border aspect-[3/4] animate-pulse" />
              ))
